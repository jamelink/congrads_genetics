...
#Rscript /home/jitame/bin/code/CONGRADS_language/04_pgs/04_mediation_CONGRADS.R
...
wd_path <- file.path(workspace_path, "projects", "CONGRADS_rest")
# set working directory
setwd(wd_path)

# set file names
pgs_fn <- file.path(workspace_path, "projects", "CONGRADS_rest", "all_scores_uncor_new.csv")
idps_fn <- file.path(workspace_path, "projects", "CONGRADS_rest", "all_idps_congrads_new.tsv")
cov_fn <- file.path(workspace_path, "projects", "CONGRADS_rest", "regenie_covariates_65k.tsv")
#beh_fn <- file.path(workspace_path, "derived_data", "phenotype_data", "cognitive", "ElsePheno_participant.tsv")
beh_fn <- file.path(workspace_path,  "projects", "CONGRADS_rest", "65k_ukbid.tsv")
#--------------------------------------#
### 1. LOAD DATA ###
#--------------------------------------#
...
rownames(pgs) <- as.integer(pgs$FID)
pgs$FID <- NULL
...
covariates$FID <- NULL
covariates$IID <- NULL
covariates$site_dummy_11025 <- as.integer(as.logical(covariates$site_dummy_11025))
covariates$site_dummy_11026 <- as.integer(as.logical(covariates$site_dummy_11026))
covariates$site_dummy_11027 <- as.integer(as.logical(covariates$site_dummy_11027))
covariates$site_dummy_11028 <- as.integer(as.logical(covariates$site_dummy_11028))
...
brain_data <- as.data.frame(cbind(pgs_centered, idps, covariates))
...
brain_data <- brain_data[complete.cases(brain_data), ]
print(paste("Dimensions dataframe after removing missing values all data df:", dim(all_data)))
print(paste("Dimensions dataframe after removing missing values brain data df:", dim(brain_data)))
...
results_df <- data.frame(
    ...
    a_t =  numeric(),
    a_pvalue = numeric(),
    a_brain_effect = numeric(),
    a_brain_se =  numeric(),
    a_brain_t =  numeric(),
    a_brain_pvalue = numeric(),
    b_effect = numeric(), 
    b_se = numeric(),
    b_t = numeric(),
    b_pvalue = numeric(),
    ...
    c_t = numeric(),
    c_pvalue = numeric(),
    c_prime_effect = numeric(),
    c_prime_se = numeric(),
    c_prime_t = numeric(),
    c_prime_pvalue = numeric(),
...
for (pgx in pgs_nms) { for (idp in idp_nms) {

    print(paste("Running mediation analysis for PGS:", pgx, "IDP:", idp, "Behavior:", beh))
    ...
    a_brain <- glm(formula_a, data=brain_data)
    
    # run mediation analysis  
    med.out <- mediate(a, c_prime, treat=pgx, mediator=idp, covariates=cov_nms, sims=1000, boot=TRUE)
    ...
    results_df <- rbind(results_df, data.frame(
        ...
        Behavior = beh,
        a_effect = a$coef[2],
        a_se = coef(summary(a))[,'Std. Error'][2],
        a_t = coef(summary(a))[,'t value'][2],
        a_pvalue = coef(summary(a))[,'Pr(>|t|)'][2],
        a_brain_effect = a_brain$coef[2],
        a_brain_se = coef(summary(a_brain))[,'Std. Error'][2],
        a_brain_t = coef(summary(a_brain))[,'t value'][2],
        a_brain_pvalue = coef(summary(a_brain))[,'Pr(>|t|)'][2],
        b_effect = b$coef[2],
        b_se = coef(summary(b))[,'Std. Error'][2],
        b_t = coef(summary(b))[,'t value'][2],
        b_pvalue = coef(summary(b))[,'Pr(>|t|)'][2],
        c_effect = c$coef[2],
        c_se = coef(summary(c))[,'Std. Error'][2],
        c_t = coef(summary(c))[,'t value'][2],
        c_pvalue = coef(summary(c))[,'Pr(>|t|)'][2],
...
write_csv(results_df, file.path(workspace_path, "projects", "CONGRADS_rest", "mediation_results_cmap_NEW.csv"))
