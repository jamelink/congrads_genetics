...
if [ -z ${chr} ] ; then
	echo -e "\nERROR: Required input arguments are missing. Specify a chromosome number when running the script.";
	usage
fi
...
RunName=CONGRADS
DataPath=/data/workspaces/lag/workspaces/lg-ukbiobank/primary_data/genetic_data/snp/snp_release_v3/imp
#SubjectList=/data/clusterfs/lag/users/jitame/CONGRADS/pheno/participant_plink_N34557.txt
#SubjectList=/data/workspaces/lag/workspaces/lg-ukbiobank/projects/CONGRADS_rest/participant_list_imaging_gen_check_65k_N49345.txt
SubjectList=/data/clusterfs/lag/users/jitame/CONGRADS/pheno/congrads_final_subs_N46267.txt
OutDir=/data/clusterfs/lag/users/jitame/CONGRADS/geno/regenie/st2_in_gwas
...
if [ -f ${OutDir}/${RunName}_chr${chr}.bgen ]; then
	...
	${qctoolDir}/qctool \
	-g ${DataPath}/ukb_imp_chrX_v3.bgen \
	-s ${DataPath}/ukb16066_imp_chrX_v3_s486743.sample \
	...
	printf "Subset BGEN file for chr XY, including samples in "${SubjectList}"\n\n"
	${qctoolDir}/qctool \
	-g ${DataPath}/ukb_imp_chrXY_v3.bgen \
	-s ${DataPath}/ukb16066_imp_chrXY_v3_s486429.sample \
	-incl-samples ${SubjectList} \
	-ofiletype "bgen_v1.2" \
	-og ${OutDir}/${RunName}_chr${chr}.bgen \
	-log ${OutDir}/${RunName}_chr${chr}.bgen.log
	...
	${qctoolDir}/qctool \
	-g ${DataPath}/ukb_imp_chr${chr}_v3.bgen \
	-s ${DataPath}/ukb16066_imp_chr1_v3_s487395.sample \
	-incl-samples ${SubjectList} \
	-ofiletype "bgen_v1.2" \
	-og ${OutDir}/${RunName}_chr${chr}.bgen \
	-log ${OutDir}/${RunName}_chr${chr}.bgen.log
	...
	printf "\nERROR: Input chromosome number incorrect. Possible values for chromosome are 1-22, X or XY.\n"
...
elif [ ${chr} -ge 1 ] && [ ${chr} -le 22 ] || [ ${chr} == "X" ] || [ ${chr} == "XY" ]; then

	${bgenixDir}/bgenix -g ${OutDir}/${RunName}_chr${chr}.bgen -index

else

	printf "\nERROR: Input chromosome number incorrect. Possible values for chromosome are 1-22 or X or XY.\n"
...
if [ -f ${OutDir}/${RunName}_chr${chr}.sample ]; then
	...
	awk '(NR==FNR){arr[$1];next}($1 in arr)' ${OutDir}/${RunName}_chr${chr}_.sample ${DataPath}/ukb16066_imp_chrX_v3_s486743.sample > ${OutDir}/${RunName}_chr${chr}.sample
